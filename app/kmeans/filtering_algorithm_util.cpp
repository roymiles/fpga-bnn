...
coord_type_ext tree_adder(coord_type_ext *input_array)
{
    ...
    for(uint j=0;j<MYCEILLOG2[DATA_DIMENSIONS];j++)
    {
        #pragma HLS unroll
        #pragma HLS dependence variable=input_array inter false
        if (j<MYCEILLOG2[DATA_DIMENSIONS]-1) {
            for(uint i = 0; i < uint(DATA_DIMENSIONS/(1<<(j+1))); i++)
            {
                #pragma HLS unroll
                #pragma HLS dependence variable=input_array inter false
            ...
            if (DATA_DIMENSIONS > uint(DATA_DIMENSIONS/(1<<(j+1)))*(1<<(j+1)) ) {
                input_array[uint(DATA_DIMENSIONS/(1<<(j+1)))] = input_array[uint(DATA_DIMENSIONS/(1<<(j+1))-1)*2+2];
            }
        }
        if (j== MYCEILLOG2[DATA_DIMENSIONS]-1) {
...
void dot_product(data_type_ext p1,data_type_ext p2, coord_type_ext *r)
{
    ...
    coord_type_ext tmp_mul_res[DATA_DIMENSIONS];
...
void tooFar_fi(data_type closest_cand, data_type cand, data_type bnd_lo, data_type bnd_hi, bool *too_far)
{
    ...
    coord_type_ext tmp_mul_res[DATA_DIMENSIONS];
    coord_type_ext tmp_mul_res2[DATA_DIMENSIONS];
