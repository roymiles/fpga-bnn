...
#define DATA_DIMENSIONS 3         // data dimensionality (D)
#define MAX_DATA_POINTS 32768     // max number of data points (N)
#define MAX_CENTRES 256       // max number of centres (K)
#define MAX_OUTER_ITERATIONS 6         // max number of outer clustering iterations (L)
...
#define HEAP_SIZE 2*MAX_DATA_POINTS       // max size of heap memory for the kd-tree (2*n tree nodes)
...
typedef ap_int<DATA_DIMENSIONS*COORD_BITWIDTH> coord_type_vector;
typedef ap_int<COORD_BITWITDH_EXT> coord_type_ext;
typedef ap_int<DATA_DIMENSIONS*COORD_BITWITDH_EXT> coord_type_vector_ext;
...
struct centre_heap_type {
    centre_index_type idx[MAX_CENTRES];
};
