...
bool read_data_points(uint n, uint k, double std_dev, data_type* points, uint* index)
{
    ...
    FILE *fp;
    char filename[256];
    make_data_points_file_name(filename,n,k,DATA_DIMENSIONS,std_dev);
    fp=fopen(filename, "r");
    ...
    for (uint j=0; j<DATA_DIMENSIONS; j++) {
        for (uint i=0;i<n;i++) {
...
bool read_initial_centres(uint n, uint k, double std_dev, data_type *initial_centre_positions, uint* centr_idx)
{
    ...
    FILE *fp;
    char filename[256];
    make_initial_centres_file_name(filename,n,k,DATA_DIMENSIONS,std_dev,FILE_INDEX);
    fp=fopen(filename, "r");
    if (fp == 0) {
        printf("failed to open file\n");
        return false;
...
node_pointer buildkdTree(data_type* points, uint *idx, uint n, data_type *bnd_lo, data_type *bnd_hi, node_pointer root_offset, kdTree_type *heap)
{
    ...
    stack_record bt_stack_array[MAX_DATA_POINTS]; //STACK_SIZE=N
...
void write_tree_node_to_file(kdTree_type u, node_pointer idx, FILE *fp)
{
    ...
    fprintf(fp,"%d\n",(int)get_coord_type_vector_ext_item(u.wgtCent.value,DATA_DIMENSIONS-1));
}
...
void readout_tree(bool write2file, uint n, uint k, double std_dev, node_pointer root, kdTree_type *heap, uint offset, kdTree_ptr image, node_pointer *image_addr)
{
    ...
    if (write2file) {
        ...
        char filename[256];
        make_tree_data_file_name(filename,n,k,DATA_DIMENSIONS,std_dev);
        if (offset == 0)
            fp = fopen(filename, "w");
        else
            fp = fopen(filename, "a");
